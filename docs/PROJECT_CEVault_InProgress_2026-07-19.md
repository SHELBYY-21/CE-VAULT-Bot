...
| Hosting | Netlify / self-host (Node host ที่รองรับ Next.js) | auto-deploy จาก `main` · ตั้ง webhook ไปที่ `/api/telegram/webhook` |
...
|---|---|
| Deploy production | `git push origin main` (Netlify auto-deploy) หรือ build/host Next.js เอง แล้ว `setWebhook` ไปที่ `APP_URL/api/telegram/webhook` |
